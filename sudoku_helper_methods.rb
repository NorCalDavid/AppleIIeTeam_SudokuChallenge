...
test.each_index do |index|
  class_instances << SudokuCell.new(test[index], @value)
end
array_of_values = class_instances.map { |cell| cell.value}
rows_of_values = []
9.times do
  rows_of_values << array_of_values.shift(9)
end
return rows_of_values
end
